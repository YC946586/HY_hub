...
namespace UninstallPackage
{
    public class LocalUnInstallTesting
    {
        ...
        /// 显示名称
        /// </summary>
        public const string DisplayName = "黑鹰 Hub客户端";
        /// <summary>
        /// 删除注册表
        /// </summary>
            ...
            try
            {
                ...
                if (Directory.Exists(file))
                {
                    foreach (string f in Directory.GetFileSystemEntries(file))
                    {
                        if (File.Exists(f))
                        {
                            if (f.Contains("HY.UninstallPackage"))
                            {
                                continue;
                            }
                            File.Delete(f);
        ...
        public static void DelMenuLnk()
        {
            ...
            if (System.IO.File.Exists(desktop + "\\黑鹰 Hub 平台\\" + DisplayName + ".lnk"))
            {
                System.IO.File.Delete(desktop + "\\黑鹰 Hub 平台\\" + DisplayName + ".lnk");
            }
            if (System.IO.File.Exists(desktop + "\\黑鹰 Hub 平台\\卸载黑鹰 Hub 客户端.lnk"))
            {
                System.IO.File.Delete(desktop + "\\黑鹰 Hub 平台\\卸载黑鹰 Hub 客户端.lnk");
            }
            if (Directory.Exists(desktop + "\\黑鹰 Hub 平台"))
            {
                Directory.Delete(desktop + "\\黑鹰 Hub 平台");
            }
        }
    }
}
